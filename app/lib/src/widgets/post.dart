...
class _PostWState extends State<PostW> {
  ...
  void likePost(int like)
  {
      switch(widget.postType)
      {
        ...
        {
          if(like == 0){
          clubProvider.likePost(widget.postId, userId).then(
            (val)
            {
              setState(() {
          ...
          {
            clubProvider.dislike(widget.postId, userId).then(
            (val)
            {
              setState(() {
                likes2 = Set<String>.from(val);
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        LikeableImage(imageUrl: widget.imgUrl, handleLike: likePost,),
        /*Image.network(
          // Replace with your image URL
          imgUrl,
          fit: BoxFit.cover,
                      ...
                      onPressed: (){
                        showModalBottomSheet(
                          ...
                          builder: (BuildContext context) {
                            return CommentModal(postType: widget.postType, postId: widget.postId);
                          },
        ...
        child: Text(likeIcon == liked ? ('You ${likes2.length-1 > 0 ? ' and ${likes2.length-1} others ' : 'like this post'}'): '${likes2.length} likes',
